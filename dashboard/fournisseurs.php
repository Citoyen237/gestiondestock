 ...
 $delete="";
  if(isset($_POST['modifier'])){
    //var_dump("sss");
     $idd=$_POST['id'];
     $_SESSION['id_f']=$idd;
     header('location:UpdateFournnisseur.php');
  }
  ...
  if(isset($_POST['supprimer'])){
    $idd=$_POST['id'];
   // $sql2="DELETE FROM `fournisseurs` WHERE `id_f`='$idd'";
...
<head>
  ...
  <title>Gerer fournisseurs</title>
      ...
      <div class="card">
        <div class="card-body">
          <h5 class="card-title">Fournisseurs
            <a href="createFournisseur.php" class="btn btn-primary">Ajouter</a>
...
$premier=($currentPage*$parPage)-$parPage;
  $sql="SELECT * FROM `fournisseurs`  limit $premier,$parPage"; 
$re=$con->query($sql);
...
while($row=mysqli_fetch_assoc($re)){
                  ...
                  $nom=$row['nom_f'];
                  $prenom=$row['prenom_f'];
                  $telephone=$row['telephone_f'];
                  $email=$row['email_f'];
                  $adresse=$row['adresse_f'];
                  $date=$row['date_cr'];
                  echo"
                    <tr>
                    <th scope='row'>$i</th>
                    ...
                    <form method='post' action='fournisseurs.php'><button type='submit'  class='btn btn-primary' name='modifier'><i class='bi bi-pencil-square'></i></button>
                    <button type='submit'  class=' btn btn-danger' name='supprimer'><i class='bi bi-trash-fill'></i></button>
                    <input type='hidden' value='$id' name='id'> </td></form>
                  </tr>
                    ";
                    $i++;
}
