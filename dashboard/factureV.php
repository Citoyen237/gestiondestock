  ...
  class myPDF extends FPDF{
   // $idfac=10;
    function header(){
        ...
        $this->Cell(276,10,'Nom du fournisseur : '.$nomd,0,0,'L');
        $this->SetXY(130,57);
        $this->Cell(276,10,'Num Facture : A00'.$numfac,0,0,'');
        $this->Ln();
        $this->SetFont('Times','BI',17);
    ...
    function viewTable($con){
      ...
      $sql="SELECT * FROM `factures`WHERE `num_fac`='$idsa' ";
                       ...
                       while($row=mysqli_fetch_assoc($re)){
                        $id=$row['id_fac'];
                        $numfac=$row['num_fac'];
                        $sql5="SELECT * FROM `achats` WHERE `codeFac`='$idsa' ";
                        $quere=$con->query($sql5) or die($con->error);
                        while($rs=mysqli_fetch_assoc($quere)){
                            $qua=$rs['quandite_a'];
                            $pru=$rs['prix_a'];
   ...
   // $this->SetTextColor(0 , 80, 180);
    ...
    $this->Cell(180,10,'Signature fournisseur                                          Signature caisse',0,1,'J');
